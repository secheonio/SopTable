...
const swaggerSpec = swaggerJSDoc({
  ...
  apis: [__filename], // index.js 내 JSDoc 기반
});
...
// 수업조사표/수업계획서/시간표/수강신청/학생부 제목 API 라우트 등록
 ...
 * /api/role-menu-log:
 *   get:
 *     summary: 권한 변경 이력 로그 조회
 *     description: 권한별 메뉴 변경 이력 로그를 반환합니다.
 *     responses:
 *       200:
 *         description: 로그 목록
 *         content:
 ...
 *                 type: object
 */
app.get('/api/role-menu-log', (req, res) => {
  const logPath = path.join(__dirname, 'roleMenuLog.json');
  fs.readFile(logPath, 'utf8', (err, data) => {
    if (err) return res.status(500).json({ error: '로그 파일을 읽을 수 없습니다.' });
    try {
      const logs = JSON.parse(data);
      res.json(logs);
    } catch {
      res.status(500).json({ error: '로그 파일 파싱 오류' });
    }
  });
});
...
app.post('/api/users/batch-upsert', batchUpsert);
...
// POST: 권한별 메뉴 설정 저장 (변경 이력 로그 기록)
/**
 * @swagger
 * /api/role-menu:
 *   post:
 *     summary: 권한별 메뉴 설정 저장
 *     description: 권한별 메뉴(메뉴별 세부권한) 설정을 저장합니다. 변경 이력 로그도 기록됩니다.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
